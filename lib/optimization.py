...
class SGD(Optimizer):
    ...
    def optimize_embedding(self, embedding_model, loss_function, pair_generator, fixed_vertices=set()):
        for epoch in range(self.n_epoch):
            ...
            for batch in pair_generator():
                ...
                grad = loss_function.loss_gradient(batch, embedding_model.get_distance_info(batch, fixed_vertices=fixed_vertices))
                if grad is None:
                    continue
                x -= grad * self.learning_rate
                embedding_model.set_state_vector(x, fixed_vertices)
            finish = time.time()
            if self.verbose:
